Shader "Custom/DepthOfFieldMine" 
{
	Properties
	{
	...
	ZWrite Off
		...
		Pass { // 0 circleOfConfusionPass
			CGPROGRAM
				...
				// Calulates and returns the CoC, which contains information on which parts are focussed and which parts are blurred
		...
		Pass { // 1 preFilterPass
			CGPROGRAM
				...
				half4 FragmentProgram(Interpolators i) : SV_Target {
					...
					// Take most extreme CoC value of 4 texels, either positive or negative
		...
		Pass { // 2 bokehPass
			...
			// blob/v2/PostProcessing/Shaders/Builtins/DiskKernels.hlsl
				...
				// Smoothen transition between focussed and blurred
				half Weigh(half coc, half radius) {
					return saturate((coc - radius + 2) / 2);
				}

				// Generates the bokeh effect (strength of the blur for each point)
				half4 FragmentProgram(Interpolators i) : SV_Target {
					half coc = tex2D(_MainTex, i.uv).a;

				// Previously took a 9 by 9 sample of texels and returns one color for each square, resulting in blur
				// Now takes the Kernel array's data to create circular blur for each texel
				...
				// this combines the back and foreground colors
				bgColor *= 1 / (bgWeight + (bgWeight == 0));
				fgColor *= 1 / (fgWeight + (fgWeight == 0));
				half bgfg =
					min(1, fgWeight * 3.14159265359 / kernelSampleCount); // PI because we're dealing with discs for blur
